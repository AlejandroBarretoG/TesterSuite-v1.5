...
import { uploadFile, listFiles, deleteFile, createFolder, moveFile, FileData } from '../services/storage';
...
export const StorageLab: React.FC = () => {
  ...
  const handleDelete = async (fullPath: string) => {
    if (!app || !confirm("¿Eliminar permanentemente?")) return;
                ...
                >
                  ...
                  {!isFolder && (
                    <div className="absolute top-2 right-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                       <ActionButtons file={file} onDelete={() => handleDelete(file.fullPath)} onCopy={(url: string) => handleCopyUrl(url, file.fullPath)} copiedId={copiedId} onMove={() => { setMoveTarget(file); setMoveDestInput(currentPath); }} />
                    </div>
                  )}
                </div>
              );
...
const ActionButtons = ({ file, onDelete, onCopy, copiedId, onMove }: any) => (
  <>
    ...
    <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="p-1.5 bg-white border border-slate-200 rounded hover:text-red-600 hover:border-red-200 shadow-sm" title="Eliminar">
      <Trash2 size={14} />
    </button>
  </>
);
